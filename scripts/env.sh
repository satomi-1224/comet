...
#
# このマシンには Xcode が入っておらず Command Line Tools のみのため、
# SPM は Testing.framework とその依存 lib_TestingInterop.dylib を自動では見つけられない。
# 探索パスと rpath を明示的に渡す必要がある。
#
#   Testing.framework       : $DEV_FRAMEWORKS
#   lib_TestingInterop.dylib: $DEV_LIB
#
# Xcode を入れた場合、Testing.framework の位置が変わって COMET_TEST_FLAGS が空になるが、
