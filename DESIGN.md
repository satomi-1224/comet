...
| 同時マルチモニタレイアウト | **メインディスプレイだけを制御する。** サブディスプレイのウィンドウは管理対象外にして、素の macOS と同じように自由に置ける状態を保つ（付け替えの検知と復元は必要・§7.6） |
...
ウィンドウ側の状態は常に上書き対象。追従しなければ補正し、それでも駄目なら制約として学習して
レイアウト側が譲る。

この帰結として、**利用者がウィンドウの縁をドラッグしてリサイズしたら「分割境界を動かした」と解釈する**。
リサイズされた寸法がその分割の新しい比率になり、兄弟は再計算され、間隔は設定値どおりに保たれる。
...
**制御するのはメインディスプレイ（座標原点を含むディスプレイ）だけ。**
サブディスプレイの上にあるウィンドウは `unmanaged(.otherMonitor)` として扱い、位置も
大きさも触らない。レイアウトはプライマリの `visibleFrame` に対してだけ計算するので、
管理下に置いたままだとサブディスプレイのウィンドウをメインへ引き寄せてしまう。

- どちらのモニタのものかは**ウィンドウの中心が乗っているモニタ**で決める。
  面積比だと2画面に跨がったときに境界付近で行き来して落ち着かない
- **どのモニタにも乗っていないときは管理を続ける。** 非表示ワークスペースのウィンドウは
  画面の隅へ追い込まれて中心がどのモニタからも外れるため、ここを誤ると二度と戻せない
- メイン → サブへ動かされたら管理から外す（押し戻すと利用者と綱引きになる）。
  サブ → メインへ戻ったら再評価してタイルへ戻す
- **サブディスプレイにウィンドウを持つアプリはアプリごと非表示にできない。**
  `NSRunningApplication.hide()` は全ウィンドウを消すので、サブ側まで消えてしまう
...
1. `setDesktopImageURL` は実際に壁紙を変える。**画素で確認済み**（切替後に最初に撮れた
   1枚で既に新しい壁紙。撮影1枚 ~60ms を分解能として計測）
